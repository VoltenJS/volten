...
test("Edge Unit Tests", async (t) => {
  ...
  await t.test("isEdge detection and override", () => {
    setIsEdge(null);
  ...
  await t.test("compose.ts uses slow path createDynamicMiddlewareChain in edge", async () => {
    setIsEdge(true);
    try {
  ...
  await t.test("stringifyJson.ts uses slow path JSON.stringify in edge", () => {
    setIsEdge(true);
    try {
  ...
  await t.test("routeTree.ts uses uncompiled radix tree traversal slow path in edge", () => {
    setIsEdge(true);
    try {
  ...
  await t.test("graceful fallback when Function eval is blocked/throws EvalError", async () => {
