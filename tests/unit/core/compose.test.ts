...
test("Compose Unit Tests", async (t) => {
  await t.test("empty chain resolves instantly", async () => {
    const chain = compileMiddlewareChain([]);
    await chain(makeCtx());
    assert.ok(true, "Empty chain resolved successfully");
  });

  await t.test("middleware and final handler run in sequential order", async () => {
    const trace: string[] = [];
  ...
  await t.test("middleware can short-circuit by not calling next", async () => {
    const trace: string[] = [];
  ...
  await t.test("synchronous next() works cleanly for sync middleware blocks", async () => {
  ...
  await t.test(
    "a thrown error inside a handler is routed through ctx._app.handleError",
    async () => {
  ...
  await t.test(
    "an async rejection inside a handler is routed through ctx._app.handleError",
    async () => {
      let caught: unknown = null;
      const ctx = makeCtx();
      (ctx as any)._app = {
  ...
  await t.test("handleHandlerResult handles Web Response in Node runtime", async () => {
  ...
  await t.test("handleHandlerResult handles Web Response in Edge runtime", async () => {
