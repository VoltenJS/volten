...
test("CreateServer Unit Tests", async (t) => {
  await t.test("no arguments creates a default HTTP server", () => {
    const server = createServer();
    assert.ok(server instanceof http.Server);
    assert.ok(!(server instanceof https.Server));
  });

  await t.test("passing only a request listener creates an HTTP server", () => {
  ...
  await t.test("options without https config creates an HTTP server", () => {
    const listener: http.RequestListener = (_, res) => {
      res.end("hello");
    };
    const options = {
  ...
  await t.test("options with https config creates an HTTPS server (mocked)", () => {
    const listener: http.RequestListener = (_, res) => {
      res.end("hello");
    };
    const options = {
